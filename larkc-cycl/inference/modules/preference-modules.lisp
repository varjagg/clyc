...
;; Reconstructed: body stripped from LarKC, but used at load time by check-preference-module-properties.
...
;; Java registers preference-module-print-function-trampoline as the print method,
;; which is missing-larkc 32941. The actual printer (print-preference-module) also
;; has no body. Using a simple default based on $str98 = "[PREFMOD ~a]".
(defstruct (preference-module
            (:conc-name "PREF-MOD-")
            (:constructor make-preference-module-int ())
            (:print-function preference-module-printer))
  name
  predicate
  sense
  required-pattern
...
(defun preference-module-printer (object stream depth)
  (declare (ignore depth))
  (format stream "[PREFMOD ~a]" (pref-mod-name object)))

(defun preference-module-print-function-trampoline (object stream)
  ;; Likely calls print-preference-module.
  (missing-larkc 32941))
